...
impl FmuTask {
    pub fn new(name: String, path: String) -> Result<Self, FrameworkError> {
        let fmu = Fmi::load(&path).map_err(|e| FrameworkError::LibloadingError(e.to_string()))?;
        Ok(FmuTask {
            name,
            inputs: HashMap::new(),
            outputs: HashMap::new(),
...
impl SimulationTask for FmuTask {
    async fn execute(&mut self, current_time: Duration) {
        ...
        // Assuming the FMU has a single output for now
        let current_time_f64 = current_time.as_secs_f64();
        if let Fmi::Fmi2(fmu) = &mut self.fmu {
            // Set inputs if any
            for (name, value) in &self.inputs {
                if let Some(vr) = fmu.get_variable_by_name(name) {
                    fmu.set_real(&[vr.value_reference], &[*value]).map_err(|e| FrameworkError::TaskExecutionError(format!("FMU set_real error: {}", e.to_string())))?;
                }
            }
            fmu.do_step(current_time_f64, 0.0, true).map_err(|e| FrameworkError::TaskExecutionError(format!("FMU do_step error: {}", e.to_string())))?; // Assuming fixed step size for now
            // Get outputs
            for (name, value) in &mut self.outputs {
                if let Some(vr) = fmu.get_variable_by_name(name) {
                    let output_values = fmu.get_real(&[vr.value_reference]).map_err(|e| FrameworkError::TaskExecutionError(format!("FMU get_real error: {}", e.to_string())))?;
                    *value = output_values[0];
                }
            }
        }
        Ok(()) // Add Ok(()) here to satisfy the return type
    }

    fn get_inputs(&self) -> Vec<String> {
        if let Fmi::Fmi2(fmu) = &self.fmu {
            fmu.model_variables().filter(|v| v.causality == Some(fmi::fmi2::model_description::Causality::Input)).map(|v| v.name.clone()).collect()
        } else {
            Vec::new()
        }
    }
    ...
    fn get_outputs(&self) -> Vec<String> {
        if let Fmi::Fmi2(fmu) = &self.fmu {
            fmu.model_variables().filter(|v| v.causality == Some(fmi::fmi2::model_description::Causality::Output)).map(|v| v.name.clone()).collect()
        } else {
            Vec::new()
        }
    }
    ...
    fn get_parameters(&self) -> HashMap<String, Parameter> {
        ...
        if let Fmi::Fmi2(fmu) = &self.fmu {
            for var in fmu.model_variables().filter(|v| v.variability == Some(fmi::fmi2::model_description::Variability::Tunable)) {
                if let Some(real) = var.real {
                    if let Ok(value) = fmu.get_real(&[var.value_reference]) {
                        params.insert(var.name.clone(), Parameter::Float(value[0]));
                    }
                }
            }
        }
    ...
    fn set_parameter(&mut self, name: &str, value: Parameter) -> Result<(), FrameworkError> {
        if let Fmi::Fmi2(fmu) = &mut self.fmu {
            if let Some(var) = fmu.get_variable_by_name(name) {
                if var.variability == Some(fmi::fmi2::model_description::Variability::Tunable) {
                    match value {
                        Parameter::Float(f) => {
                            fmu.set_real(&[var.value_reference], &[f]).map_err(|e| FrameworkError::TaskExecutionError(e.to_string()))?;
                            Ok(())
                        },
                        _ => Err(FrameworkError::TaskExecutionError(format!("Parameter '{}' expects a float value.", name))),
                    }
                } else {
                    Err(FrameworkError::TaskExecutionError(format!("Parameter '{}' is not tunable.", name)))
                }
            }
        } else {
            Err(FrameworkError::TaskExecutionError(format!("Parameter '{}' not found for FMU task '{}'.", name, self.name)))
        }
    }
}
...
pub async fn start_web_server(sim_graph_arc: Arc<RwLock<SimulationGraph>>) -> Result<(watch::Sender<SimulationStatus>, Arc<RwLock<SimulationData>>, tokio::task::JoinHandle<Result<(), FrameworkError>>), FrameworkError> {
    ...
    let combined_routes = warp::path!("hello")
        ...
        .or(warp::path!("graph").and(warp::any()).and_then({
            let sim_graph_for_web = Arc::clone(&sim_graph_arc);
            move || {
                let sim_graph_for_web_inner = Arc::clone(&sim_graph_for_web);
                async move {
                    let sim_graph_guard = sim_graph_for_web_inner.read().await;
        ...
        .or(warp::path!("parameters" / "set").and(warp::post()).and(warp::body::json()).and(warp::any()).and_then({ // Clone for parameters/set route
            let sim_graph_for_web = Arc::clone(&sim_graph_arc);
            move |req: SetParameterRequest| async move {
                let mut sim_graph_guard = sim_graph_for_web.write().await;
                ...
                if !found_task {
                    ...
                    // This part should not be reached if found_task is true and set_parameter returns Ok
                    Err(warp::reject::custom(FrameworkError::Unknown("Unexpected error during parameter setting.".to_string())))
                }
            }
        }));
    ...
    *sim_graph_arc.write().await = sim_graph; // Update the shared sim_graph_arc
    ...
    fn set_parameter(&mut self, name: &str, value: Parameter) -> Result<(), FrameworkError> {
        if let Fmi::Fmi2(fmu) = &mut self.fmu {
            ...
            Err(FrameworkError::TaskExecutionError("FMU not initialized.".to_string()))
        }
    }
}
